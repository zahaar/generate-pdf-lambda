...
- When deploted CloudFront does double job and once again encodes the response. Which forces devs to decode it first
- Add Authorisation Layer to API GW
- Configure API GW rate limits
- Enable API GW cache
...
1. Supply event from `api-gw-event.json` file  
   Update the [file](/events/api-gw-event.json) body with HTML string that would create a PDF.  
   in console run:  
   `make invokation-local` .  
   The response would be a base64 encoded PDF binary

2. Run API + Lambda as API server with:  
    `make api-local`
   run Postman API call with an example `curl` .  
   The response would be a binary PDF

**curl**:

```
curl --location --request POST 'http://127.0.0.1:3000/ratings' \
